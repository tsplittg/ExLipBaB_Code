...
model_string = f'abalone_net_(9x16x16x1)_GroupSort_numGroups{8}'
#model_string = f'abalone_net_(9x16x16x1)_ReLU'

# load the saved bike sharing network weights and biases
wts = np.load(f'../exlipbab_saved_networks/abalone/{model_string}_weights.npy', allow_pickle=True)

bs = np.load(f'../exlipbab_saved_networks/abalone/{model_string}_biases.npy', allow_pickle=True)
...
input_polyhedron = FullPolyhedron(9)
...
glb, gub, bound_hist = exlipbab_main(N = network, X=input_polyhedron, lower_bound = 0., record_ram = True, solver = "glpk", verbose= False, record_symprop= True)
...
np.save(f'../exlipbab_saved_networks/abalone/{model_string}_exlipbab_bound_history.npy', np.array(bound_hist, dtype=object))
# to be safe, we save the lower and upper bounds separately as well
lower_bound_history, upper_bound_history = bound_hist
np.save(f'../exlipbab_saved_networks/abalone/{model_string}_exlipbab_lower_bound_history.npy', np.array(lower_bound_history, dtype=object))
np.save(f'../exlipbab_saved_networks/abalone/{model_string}_exlipbab_upper_bound_history.npy', np.array(upper_bound_history, dtype=object))
