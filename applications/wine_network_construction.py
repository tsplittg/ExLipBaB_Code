...
network_shape = [11, 12, 12, 1]
#activation = ReLU()  
activation = GroupSort(6)
...
np.save(f'../exlipbab_saved_networks/wine/wine_net_({"x".join([str(s) for s in network_shape])})_{activation_name}_weights.npy', np.array(weights, dtype= object))
np.save(f'../exlipbab_saved_networks/wine/wine_net_({"x".join([str(s) for s in network_shape])})_{activation_name}_biases.npy', np.array(biases, dtype= object))

# also transpose the weights and save them in matlab format, note that LipSDP expects the double format
savemat(f'../exlipbab_saved_networks/wine/wine_net_({"x".join([str(s) for s in network_shape])})_{activation_name}_weights.mat', 
        {'weights': np.array([w.T.astype(np.float64) for w in weights], dtype=object)})
savemat(f'../exlipbab_saved_networks/wine/wine_net_({"x".join([str(s) for s in network_shape])})_{activation_name}_biases.mat', 
